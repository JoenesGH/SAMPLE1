...
$(document).ready(function(){
    ...
    $("#clickToClone").on('click', function(e){
        ...
        cloned.find(".prescription").html("Select Stock");
			...
			return new Promise((res, rej)=>{
				...
				for(let key in currentItems){
					...
					if(!inArray(key, selectedItemsArr)){
						...
						cloned.find(".selectedItemDefault").append("<option value='"+key+"'>"+currentItems[key]+"|"+currentItems_unit[key]+"</option>");
					}
				}
			
				//prepend 'select item' to the select option
				cloned.find(".selectedItemDefault").prepend("<option value='' selected>Select Stock</option>");
   ...
   //     var modeOfPayment = $("#modeOfPayment").val();
        var modeOfPayment = 'Cash'; // para sa mode of payment selected
...
function selectedItem(selectedNode){
    if(selectedNode){
        ...
        var prescription = selectedNode.parentNode.parentNode.children[5].children[1];
        ...
        $.ajax({
            ...
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    prescription.innerHTML = returnedData.prescription;           
                
                 //to popup prescription is required
                if(prescription.innerHTML == "prescription"){
                  $('#warning-prescription').html("<div class='alert alert-success alert-dismissible'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><h5><i class='icon fa fa-check'></i> hi!</h5>The Selected Product Required Prescription Slip.</div>");
                    setTimeout(function(){
                  $("#warning-prescription").html('');
                  },5000);
                } 
