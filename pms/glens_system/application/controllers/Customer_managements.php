...
class Customer_managements extends CI_Controller{
    ...
    */
    ...
    public function index(){
        ...
        $data['pageContent'] = $this->load->view('Customer_managements/admin', '', TRUE);
    ...
    */
    ...
    public function laad_(){
        ...
        $totalAdministrators = count($this->Customer_management->getAll());
        ...
        $config = $this->genlib->setPaginationConfig($totalAdministrators, "customer_managements/laad_", $limit, ['class'=>'lnp']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all customers from db
        $data['allAdministrators'] = $this->Customer_management->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalAdministrators > 0 ? ($start+1) . "-" . ($start + count($data['allAdministrators'])) . " of " . $totalAdministrators : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        
        $json['adminTable'] = $this->load->view('Customer_managements/adminlist', $data, TRUE);//get view with populated customers table
    ...
    */
    ...
    public function add(){
        ...
        $this->form_validation->set_rules('email', 'E-mail', ['trim', 'required', 'valid_email', 'is_unique[walkin_register.email]', 'strtolower'], 
                ['required'=>"required", 'is_unique'=>'E-mail exists']);
        $this->form_validation->set_rules('mobile1', 'Phone number', ['required', 'trim', 'numeric', 'max_length[15]', 'min_length[11]', 'is_unique[walkin_register.mobile1]'], 
                ['required'=>"required", 'is_unique'=>"This number is already Register"]);
        $this->form_validation->set_rules('mobile2', 'Other number', ['trim', 'numeric', 'max_length[15]', 'min_length[11]']);

        $this->form_validation->set_rules('vcard', 'value card', ['required', 'trim', 'numeric', 'is_unique[walkin_register.vcard]'], 
                ['required'=>"required", 'is_unique'=>"This Value Card is already Register"]);
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $inserted = $this->Customer_management->add(set_value('firstName'), set_value('lastName'), set_value('email'), set_value('mobile1'), set_value('mobile2'), set_value('vcard'));
    ...
    */
    ...
    public function update(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $updated = $this->customer_management->update($admin_id, set_value('firstName'), set_value('lastName'), set_value('email'),
    ...
    */
    ...
    public function suspend(){
        ...
        $new_status = $this->genmod->gettablecol('walkin_register', 'account_status', 'id', $admin_id) == 1 ? 0 : 1;
        
        $done = $this->Customer_management->suspend($admin_id, $new_status);
    ...
    */
    ...
    public function delete(){
        ...
        $admin_id = $this->input->post('_aId');
        $new_value = $this->genmod->gettablecol('walkin_register', 'deleted', 'id', $admin_id) == 1 ? 0 : 1;
        
        $done = $this->Customer_management->delete($admin_id, $new_value);
    ...
    */
    ...
    public function crosscheckMobile($mobile_number, $admin_id){
        ...
        $adminWithNum = $this->genmod->getTableCol('walkin_register', 'id', 'mobile1', $mobile_number);
    ...
    */
    ...
    public function crosscheckEmail($email, $admin_id){
        ...
        $adminWithEmail = $this->genmod->getTableCol('walkin_register', 'id', 'email', $email);
