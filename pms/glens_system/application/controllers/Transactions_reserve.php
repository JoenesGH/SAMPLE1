...
class Transactions_reserve extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(['transaction_reserve', 'item']);
    }
    ...
    */
    ...
    public function index(){
        ...
        $data['pageContent'] = $this->load->view('transactions_reserve/transactions', $transData, TRUE);
        $data['pageTitle'] = "Online Transactions";
    ...
    */
    ...
    public function latr_(){
        ...
        $totalTransactions = $this->transaction_reserve->totalTransactions();
        ...
        $config = $this->genlib->setPaginationConfig($totalTransactions, "transactions_reserve/latr_", $limit, ['onclick'=>'return latr_(this.href);']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all transactions from db
        $data['allTransactions'] = $this->transaction_reserve->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalTransactions > 0 ? ($start+1) . "-" . ($start + count($data['allTransactions'])) . " of " . $totalTransactions : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        
        $json['transTable'] = $this->load->view('transactions_reserve/transtable', $data, TRUE);//get view with populated transactions table
    ...
    */
    ...
    public function nso_(){
        ...
        if($allIsWell){//insert each sales order into db, generate receipt and return info to client
            ...
            $json['totalEarnedToday'] = number_format($this->transaction_reserve->totalEarnedToday());
    ...
    */
    ...
    private function insertTrToDb($arrOfItemsDetails, $_mop, $_at, $cumAmount, $_cd, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email){
        ...
        while($this->transaction_reserve->isRefExist($ref));
        ...
        foreach($arrOfItemsDetails as $get){
             ...
             */
            $transId = $this->transaction_reserve->add($itemName, $itemCode, "", $qtySold, $unitPrice, $totalPrice, $cumAmount, $_at, $_cd, 
        ...
        if($this->db->trans_status() === FALSE){
            ...
            $dateInDb = $this->genmod->getTableCol('transactions_reserve', 'transDate', 'transId', $transId);
    ...
    */
     ...
     * @return type
     */
    private function genTransReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $transDate, $_mop, $_pick, $vatAmount, $vatPercentage, 
        ...
        $transReceipt = $this->load->view('transactions_reserve/transreceipt', $data, TRUE);
    ...
    */
    ...
    public function vtr_(){
        ...
        $transInfo = $this->transaction_reserve->getTransInfo($ref);
        ...
        if($transInfo){
            ...
            $pick_date = $transInfo[0]['pick_date'];
            ...
            $json['transReceipt'] = $this->genTransReceipt($transInfo, $cumAmount, $amountTendered, $changeDue, $ref, 
                $transDate, $modeOfPayment,$pick_date, $vatAmount, $vatPercentage, $discountAmount, $discountPercentage, $cust_name,
    ...
    */
    ...
    public function report($from_date, $to_date=''){        
        ...
        $data['allTransactions'] = $this->transaction_reserve->getDateRange($from_date, $to_date);
        
        $this->load->view('transactions_reserve/transReport', $data);
    }
    ...
    public function edit(){
        ...
        $this->form_validation->set_rules('_iref', 'Transaction ID', ['required', 'trim', 'numeric']);
        $this->form_validation->set_rules('Status', 'Item Name', ['required', 'trim']);
        ...
        if($this->form_validation->run() !== FALSE){
            $itemId = set_value('_iref');
            $Status = set_value('Status');
            $custName = set_value('custName');
            $mobile = set_value('mobile');          
            //update item in db
            $updated = $this->transaction_reserve->edit($itemId, $Status, $custName, $mobile);
        ...
        if($Status == 'Ready'){
           //API ONE WAY SMS
            $sms_to = set_value('mobile');
            $sms_msg = "Your Order '$itemId' is Ready for Pickup" ;
            $user = 'API527JVSP3BH';
            $pass = 'API527JVSP3BHBNPC4';
            $sms_from = 'senderid';
        // to Send SMS Message to loyal Customer
            $sms = $this->transaction_reserve->gw_send_sms($user,$pass,$sms_from,$sms_to,$sms_msg);
            $json['status'] = $sms ? 1 : 0; 
        }
        ...
        if($Status == 'Approved'){
           //API ONE WAY SMS
            $sms_to = set_value('mobile');
            $sms_msg = "Your Order '$itemId' is Approved thank you for trusting us" ;
            $user = 'API527JVSP3BH';
            $pass = 'API527JVSP3BHBNPC4';
            $sms_from = 'senderid';
        // to Send SMS Message to loyal Customer
        ...
        if($Status == 'Cancel'){
        $ref = set_value('_iref');
        $data = $this->transaction_reserve->gw_send_sms($ref);

        foreach ($data as $get) {
                    $get->itemCode;
                    $get->quantity;
         //To minus qty from items        
        $q = "UPDATE items SET quantity = quantity + $get->quantity WHERE code = ?";
        $this->db->query($q, $get->itemCode);
        
        //API ONE WAY SMS
            $sms_to = set_value('mobile');
            $sms_msg = "Your Order '$itemId' is already cancelled" ;
            $user = 'API527JVSP3BH';
            $pass = 'API527JVSP3BHBNPC4';
            $sms_from = 'senderid';
        // to Send SMS Message to loyal Customer
